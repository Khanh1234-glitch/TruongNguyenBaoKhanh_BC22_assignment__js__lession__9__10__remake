...
function takeinfoEmployee() {
    ...
    var employee = new Employee(account, name, email, pass, datepicker, luongCB, chucvu, gioLam, gioLam1);
    return employee;
};
...
function classification() {
    if (value.length >= 192) {
        return " xuất sắc";
    } else if (value.length >= 176 && value.length < 192) {
        return " giỏi";
    } else if (value.length >= 160 && value.length < 176) {
        return " khá";
